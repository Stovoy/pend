//! Core library for the `pend` CLI tool.
//!
//! The public API intentionally mirrors the mental model of the original bash
//! scripts: *do now, wait later*.
...
/// Return the directory into which all job artefacts are written.
///
...
/// command and records artefacts. This helper is invoked by the public
/// [`do_job`] function.
...
impl JobState {
    ...
    fn poll(&mut self) -> io::Result<bool> {
        ...
        let mut read_log = |path: &Path, offset: &mut u64| -> io::Result<()> {
            if !path.exists() {
                return Ok(());
            }
